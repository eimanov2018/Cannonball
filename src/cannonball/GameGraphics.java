...
public class GameGraphics extends JPanel {
    ...
    public void paint(Graphics g) {
        super.paint(g);
        if (gameLogic.player.health > 0 && gameLogic.opponent.health > 0) {
        ...
        if (gameLogic.player.health <= 0) {
            g.drawImage(opponentWin,
                    0, 0, g.getClipBounds().width, g.getClipBounds().height,
                    0, 0, opponentWin.getWidth(null), opponentWin.getHeight(null), null);

        } else if (gameLogic.opponent.health <= 0) {
            g.drawImage(playerWin,
                    0, 0, g.getClipBounds().width, g.getClipBounds().height,
                    0, 0, playerWin.getWidth(null), playerWin.getHeight(null), null);
        }
    }
}
